...
(function() {
  ...
  var LANG_CODE =  localStorage.getItem("englishaccent");
...
function playOutput(base64_string){
 ...
 try {
  ...
  var dialog = function(state,text_msg,audio_file_url) {
    //var jwt = getJWT();
    var date = new Date();
    const url = 'https://dialogflow.googleapis.com/v2beta1/projects/'+PROJECT_ID+'/agent/sessions/'+state.config.profile.ID+':detectIntent?key='+API_KEY
    const data = {
        "queryInput": {
           "text": {"text": text_msg,
                    "languageCode": LANG_CODE 
